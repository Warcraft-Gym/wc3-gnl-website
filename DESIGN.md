...
- **Time accepts either clock form.** One input, `1:30` or `16:30`; `parseAnyClock` (`clock.mjs`) disambiguates the same way the rest of the feature does, and once it parses, the row shows both forms underneath (`"1:30 real · 16:30 day"`) so an author who thinks in day-clock time and one who thinks in the play-along stopwatch are both confirmed, not just one of them.
- **Stops reorder only on request.** Up/down arrows and a remove button move a stop; nothing reorders a row out from under the author mid-edit. "Sort by time" is a separate, explicit button — a route authored out of clock order (rare, but real — e.g. importing a route where a later pull actually happens earlier) is not silently "fixed".
- **The live readout is the same math as the detail page.** `deriveRoute` (`derive.mjs`) runs under the stop list exactly as it does on `/learn/creep-routes/<slug>` — `"Lv 3 · 758 xp at 3:25"` — so what an author sees while building is the same number a reader sees after publish, never a separate estimate.
