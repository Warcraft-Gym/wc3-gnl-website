...
- **Map is the second-level heading on every route surface (F009-followup-2).** The route name is the headline everywhere, but the map is never buried in a muted meta line under it — it gets its own line, in the display font, at full text colour, with a small thumbnail of `minimapUrl` (the plain, band-free minimap PNG, never the SVG with camp marks): a ~40px rounded thumbnail in the route page's kicker (above the `<h1>`, alongside the muted "map v2.0" and the mapVersion-mismatch warning, replacing the old "written for v2.0" meta line), a ~28px one on each list row (`RouteRow`, second line under the title) and on a map group/filter heading (`/learn/creep-routes`), and a ~22px one in the editor's "2 · Stops on \<map name\>" section heading (`RouteSubmitForm`, replacing the old map-less "Stops — click camps on the map"; the click instruction moved to a plain hint under the map itself). `CreepRoute.map` carries an optional `minimapUrl` (Sanity projection and fixtures both populate it) precisely so a route list can show this without a second `CreepMap` fetch per row.
...
- **Row anatomy.** Crest (`/factions/large/<race>.webp`) · title · a **map line** (F009-followup-2: a 28px rounded `minimapUrl` thumbnail plus the map name, display font, ~0.95× the title size, full text colour — not the muted meta line the map used to hide in) · one-line summary · a meta line reading `vs <opponent icons>` (`VsRaces`, shared with builds) · `map v<mapVersion>` when known (the name moved up to its own line, so the meta line keeps only the version) · stop count · author · up to 3 tags (`TagChip`, shared with builds), e.g. "vs Any · map v2.0 · 4 stops · by Gym coaches". The right column stacks the route's difficulty badge over a `tnum` line: the updated date and the stop count. At ~120px a per-row map thumbnail was rejected earlier (see below); 28px reads cleanly at 375px without clipping the crest/meta column or breaking the list's scan rhythm.
