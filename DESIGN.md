...
A creep route page (`/learn/creep-routes/<slug>`) shows the minimap as SVG (`CreepMap`, `src/components/creep-routes/`) beside a step table (`RouteStepTable`) — an ordered list of camp stops, no time dimension (F007: routes have no timings). The map and the table are two views of the same `CreepRoute`, and share one piece of state — the active stop — so hovering, focusing or clicking a row in the table lights the same marker on the map, and vice versa.
...
- **Marks on the map.** A camp is a filled circle in its band colour, radius scaling modestly with the camp's level; every mark carries `data-camp="<id>"`. A route is always drawn from *your* base: your own start is a red X (`--wg-loss`, `data-start="you"`), every other start is a small muted blue X (`--wg-win` at 60% opacity, `data-start="opponent"`) — wc3.no's convention, kept as a reference for harass/defend stops — no "P0"/"P1" text anywhere, on the route page or in the editor. Which start is yours is `CreepRoute.start`, an index into `map.starts` (unset/0 for every two-start map; only Turtle Rock and Twisted Meadows, with four starts each, ever need it set to something else — the editor's "Your spawn" picker only shows on those). A gold mine is a small gold diamond (a square rotated 45°), `data-mine`. Camp contents (creeps, their levels, whether they sleep) always come from the map catalogue, looked up by `campId` — never authored on the route, so a route can never invent what's in a camp.
...
- **The computed hero level.** `deriveRoute` (`src/lib/creep-routes/derive.mjs`) runs a hero through the route's camp stops in order and reports the level/xp after each stop ("Lv 2 · 306 xp"), re-reading the creep-xp reduction factor at the hero's current level on every single kill (not once per camp — see `docs/creep-routes.md`'s "XP model"); this is derived, never authored, so it can never drift from the camp data.
...
`/learn/creep-routes` (`RouteRow`, `src/components/creep-routes/`) copies `BuildRow`'s grid, so the two list pages read as one family: race crest · title + summary + meta line · a right-hand column with the level badge and an updated-date/stop-count line, same breakpoints, same `panel` treatment, same left-edge accent bar (by `level` here — `standard` gold, `beginner` win-green — where `BuildRow` keys the same accent off `difficulty`).
...
- **A stop row never asks for camp contents.** A camp stop shows the camp id, its band dot and a one-line creep summary (`"1× Sasquatch, 2× Wolf"`) pulled straight from the catalogue — read-only, the same rule the detail page follows ("camp contents always come from the catalogue, never authored on the route"). A base-action stop (`+ Base action`, `campId: null`) is the one row that takes free text, naming the action ("TP home", "buy from shop", "take the expansion").
...
- **Stops reorder only on request.** Up/down arrows and a remove button move a stop; nothing reorders a row out from under the author mid-edit — order is the only thing that matters, so it is never silently "fixed" for the author.
- **The live readout is the same math as the detail page.** `deriveRoute` (`derive.mjs`) runs under the stop list exactly as it does on `/learn/creep-routes/<slug>` — `"Lv 3 · 758 xp after 5 stops"` — so what an author sees while building is the same number a reader sees after publish, never a separate estimate.
