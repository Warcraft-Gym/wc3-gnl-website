...
export function TeamMedallions({ teams }: { teams: Team[] }) {
  return (
    <ul className="flex flex-wrap justify-center gap-x-5 gap-y-7 sm:gap-x-7">
      {teams.map((t) => (
        <li key={t.id} className="w-[7rem] sm:w-24">
          <Link
            href={`/gnl/teams/${t.slug}`}
            className="group flex flex-col items-center text-center"
          >
            <span className="grid size-[7rem] place-items-center rounded-full border-2 border-gold-deep/70 bg-surface/80 p-1 shadow-[0_0_0_4px_rgba(0,0,0,.6),0_12px_30px_-10px_rgba(0,0,0,.9)] transition-[border-color,box-shadow,transform] duration-[var(--wg-dur)] ease-[var(--ease-out-expo)] group-hover:-translate-y-1 group-hover:border-gold group-hover:shadow-[0_0_0_4px_rgba(0,0,0,.6),0_0_34px_-6px_var(--wg-gold-glow)] sm:size-24">
