...
/** MMR over the ladder season as a small SVG line, no client JS. */
export function MmrChart({ points }: { points: W3cTimelinePoint[] }) {
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="mt-2 h-32 w-full" role="img" aria-label={`MMR from ${first.mmr} to ${last.mmr}`}>
        <defs>
