...
export type MapCampDrop = {
  ...
  id?: string;
  chance: number;
  /** How many of this pool the camp drops. Two creeps each carrying a Power
   *  Up 1 means `count: 2` and two items on the ground — the camp-level list
   *  groups by pool but no longer loses the multiplicity. */
  count: number;
  items: { id: string; name: string; icon: string }[];
};

/** What a single creep (or `count` identical creeps) in a camp drops:
 *  `class` + `level` for a random pool, `id` for a concrete item. No
 *  `items[]` — resolving a pool to its item list is the camp-level
 *  `MapCampDrop`'s job, and repeating it per creep would bloat every
 *  catalogue for no gain. */
export type MapCampCreepDrop = {
  kind: "class" | "item";
  class?: string;
  level?: number;
  id?: string;
...
/** One creep type within a camp. Creeps are grouped by type *and* by what
 *  they drop, so "two Trappers, both holding a Power Up 1" is one row with
 *  `count: 2`, while a Trapper holding the permanent and another holding
 *  nothing stay separate rows — which of the five creeps carries the item
 *  is exactly the question a route author asks. */
export type MapCampCreep = {
  id: string;
  name: string;
  level: number;
  count: number;
  icon: string;
  drops?: MapCampCreepDrop[];
};
 ...
 *  of every creep's possible item drops, grouped with a `count` — see
 *  `MapCampDrop`, and `creeps[].drops` for the per-creep attribution. */
export type MapCamp = {
  id: string;
  x: number;
  y: number;
