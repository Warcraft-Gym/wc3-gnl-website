...
type RawMatch = {
  ...
  mapName: string;
  teams: { players: { battleTag: string; name: string; race: number; won: boolean; oldMmr: number; currentMmr: number; mmrGain: number }[] }[];
};
...
export async function getW3cProfile(battleTag: string): Promise<W3cProfile | null> {
  ...
  const [modes, search] = await Promise.all([
    ...
    w3c<{ matches: RawMatch[] }>(`/matches/search?playerId=${tag}&gateway=${GATEWAY}&season=${season}&pageSize=10&offset=0`),
  ]);
  ...
  const matches: W3cMatch[] = (search?.matches ?? [])
    .map((m) => {
      const all = m.teams.flatMap((t) => t.players);
      const me = all.find((p) => p.battleTag.toLowerCase() === battleTag.toLowerCase());
      const them = all.find((p) => p.battleTag.toLowerCase() !== battleTag.toLowerCase());
